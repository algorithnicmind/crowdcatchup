...
async def approve_intervention(intervention_id: str, db: Session = Depends(get_db)):
    service = InterventionService(db)
    try:
...
async def reject_intervention(intervention_id: str, db: Session = Depends(get_db)):
    service = InterventionService(db)
    try:
