      ...
      <div className="flex -space-x-3">
        {visibleAvatars.map((avatar, idx) => {
          ...
          return (
            <div
              key={idx}
              className="border-4 border-[#0D0D18] rounded-full bg-[#0D0D18] transition-all duration-300 relative"
              style={{
                width: size,
                height: size,
        ...
        {extraCount > 0 && (
          <div
            className="flex items-center justify-center bg-white text-black font-semibold border-4 border-[#0D0D18] rounded-full"
            style={{
              width: size,
              height: size,
              marginLeft: -overlap,
